        ...
        if (navigator.geolocation && mapBaseInstance) {
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    console.log(
                        `Flying to user location: ${latitude}, ${longitude}`,
                    );
...
<div class="hero">
    <div class="hero-content flex-col lg:flex-row -mt-10">
        ...
        >
            <LeftContent class="max-w-sm" onExplore={handleExploreClick} />
        </div>
        <div class="relative size-[500px] flex items-center justify-center">
            {#if showMap}
                <div
                    class="absolute size-full inset-0 flex items-start justify-end"
                    in:fade={{ duration: 1200, delay: 200 }}
                    out:fade={{ duration: 1200, delay: 200 }}
                >
                    <MapBase
                        bind:this={mapBaseInstance}
                        class="size-full map-with-mask"
                        zoom={0}
