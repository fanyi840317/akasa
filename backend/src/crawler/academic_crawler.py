...
class AcademicCrawler(Crawler):
    ...
    def _create_academic_document(self, parsed_data: Dict[str, Any], doc_id: str) -> Document:
        ...
        return Document(
            ...
            chunks=chunks,
            credibility_score=parsed_data['metadata'].get('credibility_score', 0.9),
            source_type='academic',
            author=parsed_data['metadata'].get('author'),
    ...
    def _count_references(self, soup) -> int:
        ...
        # 计算有序列表中的引用项
        ol_items = references_section.find_all('li')
        return len(ol_items)
    
    def _calculate_academic_credibility(self, soup, journal: str = None, authors: str = None) -> float:
        """计算学术可信度评分"""
        base_score = 0.5
        
        # 期刊影响因子加分
        if journal:
            high_impact_journals = ['Nature', 'Science', 'Cell', 'PNAS', 'Physical Review']
            if any(j.lower() in journal.lower() for j in high_impact_journals):
                base_score += 0.3
            elif 'journal' in journal.lower() or 'proceedings' in journal.lower():
                base_score += 0.1
        ...
        # 引用数量加分
        ref_count = self._count_references(soup)
        if ref_count >= 5:
            base_score += 0.2
        elif ref_count >= 2:
            base_score += 0.1
    ...
    def _extract_keywords(self, soup) -> list:
        ...
        if keywords_div:
            # 提取span标签中的关键词
            spans = keywords_div.find_all('span')
            for span in spans:
                keyword = span.get_text().strip().rstrip(',')
                if keyword:
