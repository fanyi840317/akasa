...
class Neo4jStorage(BaseTool):
    ...
    def _run(self, operation: str, data: Optional[str] = None, query: Optional[str] = None) -> str:
        """执行数据操作
        ...
        try:
            ...
            elif operation == "query" and query is not None:
                return self._query_data(query)
            elif operation == "update" and data is not None:
                return self._update_data(data)
            elif operation == "delete" and data is not None:
                return self._delete_data(data)
            else:
                return json.dumps({"error": f"Invalid operation or missing required parameters"})
                
        except Exception as e:
            logger.error(f"Error in graph storage operation: {str(e)}")
...
class ElasticsearchStorage(BaseTool):
    ...
    def _initialize_connection(self):
        ...
        try:
            ...
            self.client = None
    def _run(self, operation: str, index: str = "mystery_events", data: Optional[str] = None, query: Optional[str] = None) -> str:
        """执行数据操作
        ...
        """
        try:
            if operation == "index" and data is not None:
                return self._index_data(index, data)
            elif operation == "search" and query is not None:
            ...
            logger.error(f"Error in storage operation: {str(e)}")
            return json.dumps({"error": str(e)})
        """运行Elasticsearch操作
