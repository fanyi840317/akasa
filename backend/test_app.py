...
from server.routes.chat import _make_event, _astream_workflow_generator
from server.mcp_request import MCPServerMetadataRequest
from server.rag_request import RAGResourceRequest
...
#         },
#     )
#     @patch("server.routes.audio.VolcengineTTS")
...
#     @patch("server.routes.audio.build_podcast_graph")
...
#     @patch("server.routes.generation.build_ppt_graph")
...
#     @patch("server.routes.generation.build_prompt_enhancer_graph")
...
#     @patch("server.routes.mcp.load_mcp_tools")
...
#     @patch("server.routes.config.SELECTED_RAG_PROVIDER", "test_provider")
...
#     @patch("server.routes.config.build_retriever")
...
#     @patch("server.routes.chat.graph")
...
class TestAstreamWorkflowGenerator:
    ...
    async def test_astream_workflow_generator_basic_flow(self, mock_graph):
        ...
        async for event in generator:
            print(f"DEBUG: {event}")  # 直接打印用于调试
            logger.info(f"Event: {event}")  # 使用配置的logger
            events.append(event)
...
#     @patch("server.routes.generation.build_prose_graph")
