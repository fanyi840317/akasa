...
<ScrollArea class="h-content bg-base-200 flex rounded-2xl border">
	{#if isLoading}
	...
	{:else if event}
		...
		<div
			class="mx-auto flex w-full max-w-6xl flex-col gap-5 p-6 sm:p-9 [@media(min-width:1700px)]:max-w-[1600px]"
		>
			<Card.Root class="h-[400px] w-full border-0 p-0">
				<!-- <BlocksuiteEditor initialJsonContent={event.content} readonly={true} /> -->
			</Card.Root>
			<div class="flex flex-col gap-4">
				<h1 class="text-sm font-semibold">Summary</h1>
				<p class="text-muted-foreground text-sm">{event.summary || 'No summary provided.'}</p>
			</div>
		</div>
