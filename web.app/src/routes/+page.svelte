...
<ScrollArea class="">
    ...
    <div class="w-full h-[90vh] flex-1 flex flex-col relative bg-card">
        ...
        <div class="flex flex-row mt-10 mx-40 relative items-center justify-center">
          <!-- <Marquee> -->
          ...
          rounded-xl bg-gradient-to-t backdrop-blur-[4px] transition-all duration-300"
              ...
              <div class="flex flex-col flex-1 h-full items-start justify-center gap-y-1 mt-0.5">
      ...
      <!-- <div class="absolute right-20 top-20">
        ...
        let:motion
      >
        <div use:motion class="rounded-lg overflow-hidden shadow-md bg-gradient-to-br from-blue-900 to-purple-900 p-2 w-40">
        
          <div class="relative flex justify-center mb-2">
            {#if contributors.length > 0}
              <div class="flex justify-center relative">
                {#each contributors.slice(0, 3) as contributor, i (contributor.id)}
                  <Avatar class="h-8 w-8 border border-white bg-gradient-to-br from-blue-400 to-purple-500" style="transform: translateX({i * -4}px); z-index: {3-i}; box-shadow: 0 2px 3px rgba(0, 0, 0, 0.1);">
                    <AvatarImage src={contributor.avatarUrl} alt="Contributor Avatar" />
                    <AvatarFallback class="text-[10px]">{contributor.id.substring(0, 1).toUpperCase()}</AvatarFallback>
                  </Avatar>
                {/each}
              </div>
            {/if}
            <div class="absolute -bottom-1 left-1/2 transform -translate-x-1/2 bg-purple-600 text-white text-[8px] px-1.5 py-0.5 rounded-full">探索者</div>
          </div>
          
          <div class="bg-gradient-to-r from-blue-800/50 to-purple-800/50 rounded-md p-1.5 backdrop-blur-sm mt-1">
            <div class="flex flex-col items-center text-center">
              <div class="flex items-baseline gap-1 mb-1.5">
                <span class="text-lg font-bold text-white tracking-tight">{Math.round($animatedTotalContributors) || '...'}</span>
                <span class="text-[10px] text-gray-300 font-medium">位探索者</span>
              </div>
              <div class="flex items-baseline gap-1 mb-1">
                <span class="text-lg font-bold text-white tracking-tight">{Math.round($animatedTotalEvents) || '...'}</span>
                <span class="text-[10px] text-gray-300 font-medium">个神秘事件</span>
              </div>
              <div class="mt-1 text-[10px] text-blue-200 flex items-center justify-center bg-blue-900/30 rounded-full py-0.5 px-2 hover:bg-blue-800/40 transition-colors cursor-pointer">
                <span>查看更多</span>
