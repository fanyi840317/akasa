    ...
    <div class="container max-w-4xl mx-auto px-4 space-y-8">
      <div class="space-y-6 text-center">
        ...
        <div class="grid grid-cols-3 gap-4 mt-8">
          <div
            class="flex items-center gap-4 p-6 rounded-lg bg-background/80 backdrop-blur-md"
          >
            <Shield class="w-8 h-8 text-primary" />
            <div>
              <div class="text-sm font-medium text-muted-foreground">
                {$_("events.credibility")}
              </div>
          ...
          >
            <div
              class="w-8 h-8 rounded-full flex items-center justify-center bg-accent/20"
            >
              <svg
          ...
          >
            <div
              class="w-8 h-8 rounded-full flex items-center justify-center bg-secondary/20"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="w-5 h-5"
  ...
  <div class="flex-1 space-y-6">
    ...
    <div class="space-y-8">
      ...
      <Card class="p-4">
        <div class="space-y-3">
          ...
          <div class="flex items-center w-full justify-between">
            <div class="flex items-center gap-2">
              ...
              <div class="space-y-1">
                <div class="flex items-center gap-2">
                  <span
                    class="font-medium hover:text-primary cursor-pointer transition-colors"
                    >{creator.name}</span
                  >
                  <Badge variant="outline" class="bg-primary/10"
                    >{creator.badges[0]}</Badge
                  >
                </div>
                <div
          ...
          <p class="text-muted-foreground leading-relaxed py-4">
            这是一起发生在{event.location.name}的{$_(
              `events.categories.${event.category}`,
            )}事件。 事件发生于{event.occurredAt.toLocaleString()}，持续时间约4分钟。
            在此期间，共有{event.witnesses
              .length}位目击者在春熙路商圈及周边区域观察到这一现象。
      ...
      <Tabs.Root value="timeline" class="w-full">
        <Tabs.List>
          <Tabs.Trigger value="timeline" class="flex items-center gap-2">
            时间线
            <Badge variant="secondary" class="text-xs">{event.timeline.length}</Badge>
          </Tabs.Trigger>
          <Tabs.Trigger value="witnesses" class="flex items-center gap-2">
            目击者
            <Badge variant="secondary" class="text-xs">{event.witnesses.length}</Badge>
          </Tabs.Trigger>
          <Tabs.Trigger value="evidence" class="flex items-center gap-2">
            图片和视频
            <Badge variant="secondary" class="text-xs">{event.evidence.length}</Badge>
          </Tabs.Trigger>
        </Tabs.List>
        ...
        <Tabs.Content value="witnesses">
          <Card class="p-4">
            <WitnessList witnesses={event.witnesses} />
          </Card>
        </Tabs.Content>
      ...
      <div class="space-y-6 py-4">
        <!-- 评论区域 -->
        <Comments
          comments={mockComments.filter((c) => c.eventId === event.id)}
          onSubmit={(content) => {
  ...
  <div class="w-80 space-y-6">
    ...
    <Card class="p-4">
      <div class="flex items-center justify-between mb-3">
        <div class="flex items-center gap-2">
          ...
          <span class="text-sm text-muted-foreground"
            >({(event.investigators || []).length})</span
          >
        </div>
        <Button
          variant="outline"
      ...
      <div class="space-y-4">
        ...
        <!-- 调查员列表详情 -->
        <!-- <div class="space-y-2">
          {#each (event.investigators || []).slice(0, 3) as investigator}
            <div class="flex items-center justify-between p-2 rounded-lg hover:bg-accent/10 transition-colors">
              <div class="flex items-center gap-2">
    ...
    <Card class="p-4">
      ...
      <div class="space-y-4">
        {#each mockEvents.slice(0, 5) as hotEvent}
          <a
            href="/events/{hotEvent.id}"
            class="block hover:bg-accent/10 rounded-lg p-2 transition-colors"
          >
