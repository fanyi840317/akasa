<script lang="ts">
  ...
  import { Send, Check, Clock, MapPin, Calendar, Sparkles, Eye, Share2, Link, Image, Twitter, Facebook, QrCode, Copy, Upload } from "lucide-svelte";
  ...
  let shareUrl = $state("");
  let coverImage = $state(x_event?.cover_image || "");
  ...
  // 事件处理函数
  function handleDateChange(event: CustomEvent<{ date: DateValue }>, type: 'start' | 'end') {
    ...
    if (x_event) {
      if (isTimeUnknown) {
        ...
        x_event.date = startDate.toDate(getLocalTimeZone()).toISOString().split("T")[0];
      }
    }
  }
  ...
  async function handleShare(type: 'copy' | 'twitter' | 'facebook' | 'wechat') {
    ...
    switch (type) {
      case 'copy':
        ...
        break;
      case 'twitter':
        window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent(x_event?.title || '')}`);
        break;
      case 'facebook':
  ...
  async function handlePublish() {
    ...
    try {
      ...
      const eventData = {
        ...
        is_public: isPublic,
        cover_image: x_event.cover_image,
  ...
  async function handleCoverUpload(event: { currentTarget: EventTarget & HTMLInputElement }) {
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        if (x_event) {
          x_event.cover_image = coverImage;
        }
      };
  ...
  <div class="relative h-[calc(100vh-280px)] bg-gradient-to-b from-background to-background/95">
    <div class="relative h-full flex flex-col">
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-2 border-l border-border/40 pl-4">
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger>
                        <Button variant="ghost" size="icon" class="h-8 w-8" onclick={handlePreview}>
                          <Eye class="h-4 w-4" />
                        </Button>
                      </TooltipTrigger>
      ...
      <div class="flex-1 flex min-h-0">
        <!-- 左侧编辑区域 -->
        <div class="flex-1 overflow-hidden" in:fly={{ x: -20, duration: 500, delay: 400 }}>
          <div class="h-full">
