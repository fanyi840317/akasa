    ...
    <div class="relative">
      ...
      <div class="space-y-4">
        <div class="flex items-center gap-2">
          ...
          {#if item.witnessIds.length > 0}
            <div class="flex -space-x-2">
              {#each item.witnessIds as witnessId}
                {#if event.witnesses.find(w => w.id === witnessId)}
                  {@const witness = event.witnesses.find(w => w.id === witnessId)}
                  <div class="w-6 h-6 rounded-full bg-secondary/20 flex items-center justify-center text-xs" title={witness?.anonymous ? '匿名目击者' : witness?.name}>
                    {witness?.anonymous ? '?' : witness?.name?.[0]}
                  </div>
                {/if}
              {/each}
            </div>
