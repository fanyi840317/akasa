...
<div class="flex flex-col h-screen bg-gray-900 text-white relative overflow-hidden">
  <!-- Messages Area -->
  <div 
  ...
  >
    {#if !chatStarted}
      <div class="flex flex-col items-center justify-center h-full text-center" in:fly={{ y: 20, duration: 300, easing: sineOut }}>
        <div class="mb-8">
          ...
          <h1 class="text-2xl font-semibold text-gray-200">我能为您做些什么？</h1>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-2xl">
          {#each suggestedPrompts as prompt}
            <button 
              ...
              class="bg-gray-800 hover:bg-gray-700 text-gray-300 text-sm p-3 rounded-lg text-left transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              {prompt}
            </button>
          {/each}
    ...
    {#each messages as message (message.id)}
      ...
      >
        <div class="max-w-xs md:max-w-md lg:max-w-lg xl:max-w-2xl px-4 py-2 rounded-lg shadow {message.role === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-200'}">
          <p class="whitespace-pre-wrap break-words">{message.content}</p>
          <p class="text-xs mt-1 {message.role === 'user' ? 'text-blue-200' : 'text-gray-400'} text-right">
  ...
  <div 
    ...
    class="fixed bottom-0 left-0 right-0 bg-gray-900 transition-all duration-300 ease-in-out {chatStarted ? 'p-2 md:p-4' : 'p-4 md:p-8'}"
  ...
  >
    <div class="max-w-3xl mx-auto">
      ...
      {/if}
      <form on:submit|preventDefault={handleSubmit} class="flex flex-col gap-2 bg-gray-800 p-2 rounded-xl shadow-2xl">
        <!-- 工具栏 -->
        <div class="flex items-center gap-2 px-2 py-1 border-b border-gray-700">
          <div class="flex items-center gap-1">
            <button type="button" class="p-1.5 text-gray-400 hover:text-gray-200 transition-colors rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-blue-500 tooltip" data-tip="新建">
              <Plus size={18} />
            </button>
            <button type="button" class="p-1.5 text-gray-400 hover:text-gray-200 transition-colors rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-blue-500 tooltip" data-tip="搜索">
              <Search size={18} />
            </button>
            <button type="button" class="p-1.5 text-gray-400 hover:text-gray-200 transition-colors rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-blue-500 tooltip" data-tip="AI 助手">
              <Brain size={18} />
            </button>
          </div>
          <div class="flex-1 flex items-center justify-center gap-1">
            <button type="button" class="p-1.5 text-gray-400 hover:text-gray-200 transition-colors rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-blue-500 tooltip" data-tip="创意灵感">
              <Lightbulb size={18} />
            </button>
            <button type="button" class="p-1.5 text-gray-400 hover:text-gray-200 transition-colors rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-blue-500 tooltip" data-tip="图片">
              <ImageIcon size={18} />
            </button>
            <button type="button" class="p-1.5 text-gray-400 hover:text-gray-200 transition-colors rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-blue-500 tooltip" data-tip="拍照">
              <Camera size={18} />
            </button>
          </div>
          <div class="flex items-center gap-1">
            <button type="button" class="p-1.5 text-gray-400 hover:text-gray-200 transition-colors rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-blue-500 tooltip" data-tip="帮助">
              <HelpCircle size={18} />
            </button>
            <div class="relative">
              <button 
                bind:this={moreOptionsButtonRef} 
                type="button" 
                class="p-1.5 text-gray-400 hover:text-gray-200 transition-colors rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-blue-500 tooltip"
                data-tip="更多选项"
              ...
              {#if showMoreOptions}
              <div bind:this={moreOptionsMenuRef} class="absolute right-0 bottom-full mb-2 w-56 bg-gray-800 border border-gray-700 rounded-lg shadow-xl py-1.5 z-50 focus:outline-none">
                <button 
                  type="button" 
                  class="w-[calc(100%-0.75rem)] text-left flex items-center px-3.5 py-2.5 text-sm text-gray-300 hover:bg-gray-700 hover:text-white transition-colors rounded-md mx-1.5 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:ring-offset-1 focus:ring-offset-gray-800"
                  on:click={() => alert('“设置”功能待实现')}
                >
                  <Settings2 size={16} class="mr-3 text-gray-400" /> 设置
                </button>
                <button 
                  type="button" 
                  class="w-[calc(100%-0.75rem)] text-left flex items-center px-3.5 py-2.5 text-sm text-gray-300 hover:bg-gray-700 hover:text-white transition-colors rounded-md mx-1.5 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:ring-offset-1 focus:ring-offset-gray-800"
                  on:click={() => alert('“关于 Trae AI”功能待实现')}
                >
                  <Lightbulb size={16} class="mr-3 text-gray-400" /> 关于 Trae AI
                </button>
                <div class="my-1.5 border-t border-gray-700"></div>
                <button 
                  type="button" 
                  class="w-[calc(100%-0.75rem)] text-left flex items-center px-3.5 py-2.5 text-sm text-red-400 hover:bg-gray-700 hover:text-red-300 transition-colors rounded-md mx-1.5 focus:outline-none focus:ring-1 focus:ring-red-500 focus:ring-offset-1 focus:ring-offset-gray-800"
                  on:click={clearConversation}
                >
                  <Trash2 size={16} class="mr-3 text-gray-400" /> 清除当前对话
                </button>
              </div>
              {/if}
            </div>
